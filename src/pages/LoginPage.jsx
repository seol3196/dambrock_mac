...
export default function LoginPage() {
  ...
  if (!loading && user && role) return <Navigate to={homeFor(role)} replace />;
    ...
    <main className="felt-bg grid min-h-screen place-items-center px-4 py-10">
      <section className="grid w-full max-w-5xl overflow-hidden rounded-[8px] bg-white shadow-soft md:grid-cols-[1.1fr_0.9fr]">
        <div className="cork-bg relative min-h-[420px] p-8">
          <div className="absolute left-10 top-12 rotate-[-3deg] rounded-[6px] bg-yellow-100 p-6 shadow-paper">
            <p className="font-hand text-5xl">오늘의 생각</p>
            <p className="mt-2 text-stone-700">친구들과 따뜻하게 나누어요.</p>
          </div>
          <div className="absolute bottom-16 right-10 rotate-[2deg] rounded-[6px] bg-rose-100 p-5 shadow-paper">
        ...
        <form onSubmit={submit} className="flex flex-col justify-center p-8">
          ...
          <label className="mt-8 block">
            ...
            <span className="flex items-center gap-2 rounded-[8px] border border-stone-200 bg-stone-50 px-3">
              ...
              <input value={id} onChange={(e) => setId(e.target.value)} className="h-12 min-w-0 flex-1 bg-transparent outline-none" placeholder="아이디" />
            </span>
          </label>
          ...
          <button type="submit" className="mt-6 inline-flex h-12 items-center justify-center gap-2 rounded-[8px] bg-stone-900 font-bold text-white hover:bg-stone-700">
            <LogIn size={18} />
            로그인
          </button>
        </form>
